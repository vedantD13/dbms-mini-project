...
// Initialize Stripe (Use standard Stripe test key)
...
export default function Checkout() {
  ...
  // 🛡️ FLOW GUARD: If a user tries to access /checkout without selecting a movie
  if (!bookingData || !bookingData.movie) {
    toast.error("Your cart is empty. Please select a movie first.");
    return <Navigate to="/" replace />;
  }

  // 💺 Map Global Context to Local Variables
  const selectedSeats = bookingData.selectedSeats || [];
  const setSelectedSeats = (seats) => updateBooking({ selectedSeats: seats });
  
  // Mock booked seats (this will come from your backend later)
  ...
  const getSeatDetails = (seatId) => {
    ...
    if (['A', 'B'].includes(row)) return { tier: 'VIP Recliner', price: 500, bg: 'bg-amber-500', shadow: 'rgba(245,158,11,0.6)' };
    if (['C', 'D', 'E'].includes(row)) return { tier: 'Premium', price: 250, bg: 'bg-indigo-500', shadow: 'rgba(99,102,241,0.6)' };
  ...
  const handleSuccessfulPayment = (bookingId) => {
    // Generate the final receipt data using context + new ID
    setReceiptData({
      id: bookingId,
      movie: bookingData.movie || "Interstellar",
      format: bookingData.format || "IMAX 3D",
      cinema: bookingData.cinema || "CineSync Cinemas",
      date: bookingData.date || "Today",
      time: bookingData.time || "09:30 PM",
  ...
  // Cleanup & Redirect when closing receipt
  const handleCloseReceipt = () => {
    clearBooking();
    navigate('/dashboard');
  };

  // Dynamically generate the UPI URL for the QR API
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-50 pt-24 pb-12 px-6 relative font-sans selection:bg-amber-500/30">
      <Helmet>
        <title>Checkout | CineSync</title>
      </Helmet>
      
      {/* 🎟️ Premium Ticket Modal Overlay */}
      <AnimatePresence>
        {showReceipt && receiptData && (
           <motion.div 
             ...
             animate={{ opacity: 1 }} 
             exit={{ opacity: 0 }}
             className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/90 backdrop-blur-xl p-6 overflow-y-auto"
           >
             <motion.div 
               initial={{ opacity: 0, scale: 0.96, y: 10 }} 
               animate={{ opacity: 1, scale: 1, y: 0 }} 
               transition={{ duration: 0.3, ease: [0.16, 1, 0.3, 1] }}
               className="my-auto py-10 w-full"
             >
        ...
        <div className="w-full xl:w-[440px] shrink-0">
          <div className="sticky top-24 space-y-6">
            ...
            <div className="bg-slate-900 rounded-3xl overflow-hidden border border-slate-800 shadow-2xl relative">
              <div className="p-8 pb-6 border-b-2 border-dashed border-slate-800 relative">
                ...
                <p className="text-slate-400 text-sm font-medium">
                  {bookingData.cinema || "CineSync Cinemas"} • {bookingData.date || "Today"} | {bookingData.time || "09:30 PM"}
                </p>
              </div>
            ...
            <div className="bg-slate-900 rounded-2xl p-2 flex border border-slate-800 shadow-lg">
              <button onClick={() => setPaymentMethod('stripe')} className={`flex-1 py-3 text-sm font-bold rounded-xl transition-all duration-200 ease-out flex items-center justify-center gap-2 ${paymentMethod === 'stripe' ? 'bg-slate-800 text-white shadow-md' : 'text-slate-500 hover:text-slate-300'}`}>
                <CreditCard size={16} /> Credit Card
              </button>
              <button onClick={() => setPaymentMethod('upi')} className={`flex-1 py-3 text-sm font-bold rounded-xl transition-all duration-200 ease-out flex items-center justify-center gap-2 ${paymentMethod === 'upi' ? 'bg-slate-800 text-white shadow-md' : 'text-slate-500 hover:text-slate-300'}`}>
                <Smartphone size={16} /> UPI
              </button>
              <button onClick={() => setPaymentMethod('netbanking')} className={`flex-1 py-3 text-sm font-bold rounded-xl transition-all duration-200 ease-out flex items-center justify-center gap-2 ${paymentMethod === 'netbanking' ? 'bg-slate-800 text-white shadow-md' : 'text-slate-500 hover:text-slate-300'}`}>
            ...
            <div className="bg-slate-900 rounded-3xl p-8 border border-slate-800 shadow-2xl min-h-[250px] relative overflow-hidden">
              <AnimatePresence mode="wait">
                ...
                {/* UPI TAB WITH REAL DYNAMIC QR CODE */}
                {paymentMethod === 'upi' && (
                  <motion.div 
                    key="upi"
                    initial={{ opacity: 0, x: -10 }} 
                  ...
                  >
                    <div className={`w-full aspect-square max-w-[160px] mx-auto bg-white rounded-2xl p-3 flex items-center justify-center shadow-lg border-4 border-slate-800 transition-opacity duration-300 ${selectedSeats.length === 0 ? 'opacity-30' : 'opacity-100'}`}>
                      {selectedSeats.length > 0 ? (
                        <img src={qrCodeApiUrl} alt="Dynamic UPI QR Code" className="w-full h-full object-contain mix-blend-multiply" />
                      ) : (
                        <div className="text-center text-slate-400 text-xs font-bold uppercase tracking-wider">Select Seats<br/>to generate QR</div>
                      )}
                    </div>
                    
                    <p className="text-center text-xs font-medium text-slate-400">
                      {selectedSeats.length > 0 ? `Scan to pay exactly ₹${totalAmount}` : 'Please select seats first'}
                    </p>

                    <div className="space-y-3">
                      <label className="text-xs font-bold text-slate-400 uppercase tracking-widest flex justify-between">
                        <span>UPI ID / VPA</span>
                        <span className="text-slate-500 font-normal">or enter manually</span>
                      </label>
                      <input type="text" placeholder="username@upi" disabled={selectedSeats.length === 0} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-4 text-sm focus:border-amber-500 focus:ring-1 focus:ring-amber-500 outline-none transition-all duration-200 placeholder:text-slate-600 font-medium disabled:opacity-50 disabled:cursor-not-allowed" />
                    </div>
                    <button disabled={selectedSeats.length === 0} className="w-full bg-slate-800 hover:bg-slate-700 text-slate-300 py-4 rounded-xl font-bold text-lg cursor-not-allowed disabled:opacity-50 transition-colors duration-200">
                      Verify & Pay (Mock)
                    </button>
                  </motion.div>
                )}
